...
# Tariff C&P integration (Low/Medium/High) with editable sidebar rates (no calculations shown),
# intensities transparency (Elec/Gas/Water) with active band highlight, hour-scaled variable energy,
# Region display fix, and simplified Ad‑hoc (multi-line) workflow using working days (Mon–Fri).
...
FT2_TO_M2 = 0.092903
BASE_HOURS_PER_WEEK = 27.0     # spreadsheet baseline; variable energy scales by (hours / 27)
...
def _render_host_df_to_html(host_df: pd.DataFrame) -> str:
    ...
    for _, row in host_df.iterrows():
        ...
        if grand_cls:
            rows_html.append(f"<tr{grand_cls}><td>{item}</td><td>{_currency(val)}</td></tr>")
        else:
...
def export_html(host_df: pd.DataFrame | None, prod_df: pd.DataFrame | None, title="Quote") -> BytesIO:
    # Keep simple GOV.UK-like table style (headings grey, borders, readable margins)
    css = """
    <meta charset="utf-8">
    <style>
...
# Region display fix: always show value once Prison is selected
...
with tariff_cols[0]:
    workshop_usage = st.radio(
        ...
        key="workshop_usage",
        help=(
            "Low: lighting, water & heating (little machinery).  "
            "Medium: mixed light industrial / intermittent machinery.  "
            "High: machinery-heavy processes plus lighting/water/heating."
        ),
    )
with tariff_cols[1]:
...
# -----------------------------
# Sidebar — Tariffs & Overheads (editable rates; no calculations shown)
# -----------------------------
with st.sidebar:
    st.header("Tariffs & Overheads")

    # Intensities for ALL bands with active band highlighted
    ...
    def _mark(v, band):  # bold + 'selected' tag for active band
        return f"**{v}** ← selected" if USAGE_KEY == band else f"{v}"
    ...
    # Apply selected band defaults before drawing widgets (avoid StreamlitAPIException)
...
# -----------------------------
# Cost functions (hours-scaled variable energy)
# -----------------------------
def monthly_energy_costs() -> tuple[float, float]:
    """
    Electricity & Gas per month including daily charges.
    Variable kWh energy scales with hours/week input vs BASE_HOURS_PER_WEEK (27).
    Standing charges (daily) do not scale with hours.
    """
    band = TARIFF_BANDS[USAGE_KEY]
    # kWh/m²/year × area → variable energy basis
    ...
    # Scale by hours (baseline 27 h/week from spreadsheet)
    try:
...
def weekly_overheads_total() -> tuple[float, dict]:
    ...
    if st.session_state.get("maint_method", "£/m² per year").startswith("£/m² per year"):
        ...
        # % of reinstatement value → monthly
        reinstate_val = st.session_state.get("reinstate_val", 0.0)
        pct = st.session_state.get("reinstate_pct", 0.0)
        maint_m = (float(reinstate_val) * (float(pct) / 100.0)) / 12.0
...
# PRODUCTION branch
    ...
    # ---------------- B) AD‑HOC COSTS (MULTI‑LINE) — working days, concise summary ----------------
    else:
        st.caption(
            "For each product line, answer two rows:\n"
            "• **Row 1** — Item name, Units requested, Deadline (working day)\n"
        ...
        if st.button("Calculate Ad‑hoc Cost", key="calc_adhoc"):
            ...
            if local_errors:
                ...
                # --- Capacity parameters (per working day) ---
                ...
                # --- Weekly costs (for per-minute cost) ---
                overheads_weekly, _detail = weekly_overheads_total()
                inst_weekly_total = (
                    sum((s / 52) * (effective_pct / 100) for s in supervisor_salaries)
                    if not customer_covers_supervisors else 0.0
                ...
                # Avoid divide-by-zero if no capacity
                minutes_per_week_capacity = max(1e-9, num_prisoners * workshop_hours * 60.0)
                cost_per_minute = weekly_cost_total / minutes_per_week_capacity  # £ per labour minute

                # --- Per-line unit costs & per-line working days to produce (if produced alone) ---
                ...
                for ln in lines:
                    mins_per_unit = ln["mins_per_item"] * ln["pris_per_item"]  # labour minutes per finished unit
                    ...
                    # Working days available to line's deadline
                    wd_available = working_days_between(today, ln["deadline"])

                    # Days needed for this line alone with current staffing
                    if current_daily_capacity > 0:
                        wd_needed_line_alone = ceil(total_line_minutes / current_daily_capacity)
                    else:
                    ...
                    per_line.append({
                        ...
                        "total_line_minutes": total_line_minutes,
                    })

                # --- Overall job feasibility with current staffing (aggregate minutes / daily capacity) ---
                if current_daily_capacity > 0:
                    wd_needed_all = ceil(total_job_minutes / current_daily_capacity)
                else:
                    wd_needed_all = float("inf")

                # Tightest deadline among lines (most stringent)
                earliest_wd_available = min((p["wd_available"] for p in per_line), default=0)

                # If not feasible for the tightest deadline, advise extra prisoners required to hit it
                if earliest_wd_available > 0 and current_daily_capacity > 0:
                    needed_capacity = total_job_minutes / earliest_wd_available  # minutes/day required
                    minutes_per_extra_prisoner_to_deadline = daily_minutes_capacity_per_prisoner
                    current_capacity_per_day = current_daily_capacity
                    deficit_per_day = max(0.0, needed_capacity - current_capacity_per_day)
                    extra_prisoners_needed = int(max(0, math.ceil(deficit_per_day / minutes_per_extra_prisoner_to_deadline)))
                else:
                    extra_prisoners_needed = 0

                # --- Monetary results ---
                # Line totals = unit cost × units (ex/in VAT)
                total_ex_vat = 0.0
                total_inc_vat = 0.0
                for p in per_line:
                    line_total_ex = p["unit_cost_ex_vat"] * p["units"]
                    line_total_inc = p["unit_cost_inc_vat"] * p["units"]
                    p["line_total_ex_vat"] = line_total_ex
                    p["line_total_inc_vat"] = line_total_inc
                    total_ex_vat += line_total_ex
                    total_inc_vat += line_total_inc

                # --- Output: concise table (Item, Units, Unit cost, Line total) ---
                show_inc = (customer_type == "Commercial" and apply_vat)
                col_headers = ["Item", "Units", "Unit Cost (£" + (" inc VAT" if show_inc else "") + ")", "Line Total (£" + (" inc VAT" if show_inc else "") + ")"]
                data_rows = []
                for p in per_line:
                    data_rows.append([
                        p["name"],
                        f"{p['units']:,}",
                        f"{p['unit_cost_inc_vat' if show_inc else 'unit_cost_ex_vat']:.2f}",
                        f"{p['line_total_inc_vat' if show_inc else 'line_total_ex_vat']:.2f}",
                    ])
                ...
                # Summary export (CSV/HTML)
                summary_df = pd.DataFrame([
                    {
                        "Item": p["name"],
                        "Units": p["units"],
                        ("Unit Cost inc VAT (£)" if show_inc else "Unit Cost (£)"):
                            round(p["unit_cost_inc_vat" if show_inc else "unit_cost_ex_vat"], 2),
                        ("Line Total inc VAT (£)" if show_inc else "Line Total (£)"):
                            round(p["line_total_inc_vat" if show_inc else "line_total_ex_vat"], 2),
                    }
                    for p in per_line
                ])
                totals_row = {
